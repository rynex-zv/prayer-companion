...
export async function mauiCall<T = unknown>(
  ...
  try {
    ...
    if (!handler) {
      logBridge("failure", { callId, method, source: "mock", elapsedMs: elapsed(started), error: `No mock for "${method}"` });
      return { ok: false, error: `No mock for "${method}"` };
    }
    const data = await withBridgeTimeout(Promise.resolve(handler(payload)), timeoutMs, method, "mock");
    logBridge("success", { callId, method, source: "mock", elapsedMs: elapsed(started) });
...
export const BRIDGE_MODE: "maui" | "wasm" | "mock" =
...
function canUseMockHandlers(): boolean {
  return import.meta.env.DEV || TEST.enabled;
}
