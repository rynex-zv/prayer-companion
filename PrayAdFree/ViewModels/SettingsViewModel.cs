...
public sealed class SettingsViewModel : ViewModelBase {
    ...
    private double _adhanVolume = 100;
    ...
    private string? _previewAdhanSoundKey;
    ...
    public SettingsViewModel(
        ...
        AdhanSoundItems = new ObservableCollection<AdhanSoundOptionViewModel>();
    ...
    public ObservableCollection<AdhanSoundOptionViewModel> AdhanSoundItems { get; }
    ...
    public OptionItem<AdhanReminderAlertType>? SelectedAdhanReminderAlertType {
        get => _selectedAdhanReminderAlertType;
    ...
    public OptionItem<string>? SelectedAdhanSound {
        ...
        set {
            if (SetProperty(ref _selectedAdhanSound, value)) {
    ...
    public bool WindowsBackgroundEnabled {
        ...
        set {
            if (!SetProperty(ref _windowsBackgroundEnabled, value)) {
                return;
            }

            if (_suspendSave || _applyingWindowsBackgroundOption || !_windowsBackgroundModeService.IsSupported) {
                return;
            }

            ApplyWindowsBackgroundOption(value);
        }
    }
    ...
    public double AdhanVolume {
        get => _adhanVolume;
        set {
            var clamped = Math.Clamp(value, 0, 100);
            var rounded = Math.Round(clamped, MidpointRounding.AwayFromZero);
            if (SetProperty(ref _adhanVolume, rounded)) {
                AdhanVolumeLabel = $"{(int)rounded}%";
            }
        }
    }

    public string AdhanVolumeLabel {
        get => _adhanVolumeLabel;
        set => SetProperty(ref _adhanVolumeLabel, value);
    }
    ...
    private void Load() {
        ...
        SelectedAdhanReminderAlertType = AdhanReminders.FirstOrDefault()?.AlertType
            ?? AdhanReminderAlertTypes.FirstOrDefault();
    ...
    private void Save() {
        ...
        var notifications = new NotificationSettings {
            ...
            ReminderItems = AdhanReminders
                .Select(item => new AdhanReminderItem {
                    OffsetMinutes = item.Minutes,
                    AlertType = item.AlertType?.Value ?? AdhanReminderAlertType.Adhan
                })
                ...
                .ToList(),
            ReminderOffsetsMinutes = AdhanReminders
    ...
    private void LoadAdhanReminders() {
        foreach (var reminder in AdhanReminders) {
            reminder.PropertyChanged -= OnAdhanReminderItemPropertyChanged;
        }

        AdhanReminders.Clear();
        var reminders = AdhanReminderResolver.Resolve(_settings.Notifications);
        foreach (var reminder in reminders) {
            var item = CreateAdhanReminderItem(reminder.OffsetMinutes, reminder.AlertType);
            AdhanReminders.Add(item);
        }
    ...
    private void RebuildReminderLabels() {
        foreach (var item in ImsakReminders) {
            item.Label = BuildReminderLabel(item.Minutes);
        }

        foreach (var item in IftarReminders) {
            item.Label = BuildReminderLabel(item.Minutes);
        }
    ...
    private void AddAdhanReminder() {
        if (!int.TryParse(AdhanReminderValue, out var value) || value == 0) {
            return;
        }

        var multiplier = SelectedAdhanReminderUnit?.Value ?? 1;
        var sign = SelectedAdhanReminderDirection?.Value ?? -1;
        var minutes = value * multiplier * sign;
        var alertType = SelectedAdhanReminderAlertType?.Value ?? AdhanReminderAlertType.Adhan;

        if (AdhanReminders.Any(item => item.Minutes == minutes &&
            (item.AlertType?.Value ?? AdhanReminderAlertType.Adhan) == alertType)) {
            return;
        }

        AdhanReminders.Add(CreateAdhanReminderItem(minutes, alertType));
        SortReminderList(AdhanReminders);
        RebuildAdhanReminderLabels();
        AdhanReminderValue = "";
        ScheduleSave();
    }
    ...
    private void RemoveAdhanReminder(ReminderOffsetItem? item) {
        ...
        item.PropertyChanged -= OnAdhanReminderItemPropertyChanged;
    ...
    private void SortReminderList(ObservableCollection<ReminderOffsetItem> list) {
        var ordered = list
            ...
            .ThenBy(item => item.AlertType?.Value ?? AdhanReminderAlertType.Adhan)
            .ToList();
        list.Clear();
    ...
    private string BuildAdhanReminderLabel(int minutes, AdhanReminderAlertType alertType) {
        var baseLabel = BuildReminderLabel(minutes);
        var scopeLabel = SelectedAdhanReminderScope?.Value == AdhanReminderScope.SpecificPrayer
            ? LocalizationManager.TranslatePrayer(SelectedAdhanReminderPrayer?.Value ?? PrayerId.Fajr)
            : LocalizationManager.Translate("Reminder_All");
        ...
        return $"{baseLabel} - {scopeLabel} - {typeLabel}";
    }

    private void RebuildAdhanReminderLabels() {
        foreach (var item in AdhanReminders) {
            var alertType = item.AlertType?.Value ?? AdhanReminderAlertType.Adhan;
            item.Label = BuildAdhanReminderLabel(item.Minutes, alertType);
        }
    }


    private ReminderOffsetItem CreateAdhanReminderItem(int minutes, AdhanReminderAlertType alertType) {
        var item = new ReminderOffsetItem(minutes, string.Empty, GetReminderAlertTypeOption(alertType));
        item.PropertyChanged += OnAdhanReminderItemPropertyChanged;
        item.Label = BuildAdhanReminderLabel(minutes, alertType);
        return item;
    }

    private OptionItem<AdhanReminderAlertType>? GetReminderAlertTypeOption(AdhanReminderAlertType alertType) {
        return AdhanReminderAlertTypes.FirstOrDefault(item => item.Value == alertType)
            ?? AdhanReminderAlertTypes.FirstOrDefault();
    }

    private void OnAdhanReminderItemPropertyChanged(object? sender, PropertyChangedEventArgs e) {
        if (sender is not ReminderOffsetItem item) {
            return;
        }

        if (e.PropertyName != nameof(ReminderOffsetItem.AlertType)) {
            return;
        }

        item.Label = BuildAdhanReminderLabel(item.Minutes, item.AlertType?.Value ?? AdhanReminderAlertType.Adhan);
        if (_suspendSave) {
            return;
        }
    ...
    private void BuildNotificationOptions() {
        ...
        foreach (var option in AdhanSoundLibrary.BuildOptions(_settings.Notifications, includeUseGlobal: false)) {
            AdhanSounds.Add(option);
        }
        RebuildAdhanSoundItems();
        ...
        try {
            foreach (var item in AdhanReminders) {
                item.AlertType = GetReminderAlertTypeOption(item.AlertType?.Value ?? AdhanReminderAlertType.Adhan);
            }
            RebuildAdhanReminderLabels();
        } finally {
            _suspendSave = previousSuspend;
        }
    }

    private void RebuildAdhanSoundItems() {
        AdhanSoundItems.Clear();
        foreach (var option in AdhanSounds) {
    ...
    private void SyncAdhanSoundItemStates() {
        var selectedKey = SelectedAdhanSound?.Value ?? string.Empty;
        foreach (var item in AdhanSoundItems) {
            item.IsSelected = string.Equals(item.Key, selectedKey, StringComparison.OrdinalIgnoreCase);
            item.IsPlaying = !string.IsNullOrWhiteSpace(_previewAdhanSoundKey)
                && string.Equals(item.Key, _previewAdhanSoundKey, StringComparison.OrdinalIgnoreCase);
        }
    }

    private void OnSelectAdhanSound(AdhanSoundOptionViewModel? item) {
        if (item == null) {
            return;
        }

        var option = AdhanSounds.FirstOrDefault(sound => string.Equals(sound.Value, item.Key, StringComparison.OrdinalIgnoreCase));
        if (option == null) {
            return;
        }

        SelectedAdhanSound = option;
    }

    private async Task ToggleAdhanPreviewAsync(AdhanSoundOptionViewModel? item) {
        if (item == null || _previewBusy) {
            return;
        }
        ...
        try {
            ...
            if (!string.IsNullOrWhiteSpace(_previewAdhanSoundKey) &&
                ...
                var started = await _adhanPlaybackService.PlayPreviewAsync(item.Key).ConfigureAwait(false);
                _previewAdhanSoundKey = started ? item.Key : null;
            }
        } catch (Exception ex) {
            _logger.LogException(ex, "SettingsViewModel.ToggleAdhanPreviewAsync");
        } finally {
            _previewBusy = false;
            RunOnMainThread(SyncAdhanSoundItemStates);
        }
    }
    ...
    private async Task SendTestNotificationAsync() {
        try {
            ...
            if (OperatingSystem.IsWindows()) {
                await _adhanPlaybackService.PlayPreviewAsync(effectiveKey).ConfigureAwait(false);
                return;
            }
    ...
    private void BuildAdhanOverrideOptions() {
        ...
        foreach (var option in AdhanSoundLibrary.BuildOptions(_settings.Notifications, includeUseGlobal: true)) {
            AdhanOverrideSounds.Add(option);
        }
    ...
    private void ApplyWindowsBackgroundOption(bool enabled) {
        _applyingWindowsBackgroundOption = true;
        try {
            var applied = _windowsBackgroundModeService.SetEnabled(enabled);
            if (applied == enabled) {
                return;
            }

            WindowsBackgroundEnabled = applied;
            StatusMessage = LocalizationManager.Translate("WindowsBackgroundServiceFailed");
        } catch (Exception ex) {
            _logger.LogException(ex, "SettingsViewModel.ApplyWindowsBackgroundOption");
            WindowsBackgroundEnabled = _windowsBackgroundModeService.IsEnabled();
            StatusMessage = LocalizationManager.Translate("WindowsBackgroundServiceFailed");
        } finally {
            _applyingWindowsBackgroundOption = false;
        }
    }
        ...
        try {
            ...
            var pick = await FilePicker.Default.PickAsync(new PickOptions {
                ...
                FileTypes = new FilePickerFileType(new Dictionary<DevicePlatform, IEnumerable<string>> {
                    { DevicePlatform.Android, new[] { "audio/mpeg", "audio/mp3", "audio/wav", "audio/aac", "audio/ogg", "audio/x-m4a" } },
            ...
            var extension = Path.GetExtension(pick.FileName);
            if (string.IsNullOrWhiteSpace(extension)) {
                extension = ".mp3";
            }

            var key = $"adhan_custom_{Guid.NewGuid():N}";
            var fileName = $"{key}{extension.ToLowerInvariant()}";
    ...
    private static bool ShouldAutoSave(string? propertyName) {
        return propertyName is nameof(UseGps)
            ...
            or nameof(SelectedAdhanReminderAlertType)
